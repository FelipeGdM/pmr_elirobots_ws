...
def build_state(
    qpos: Tensor, qvel: Tensor, tcp_pose: Tensor, goal_pos: Tensor, obj_pose: Tensor
):
...
def compute_tcp_pose(robot: DHRobot, qpos: Tensor) -> Tensor:
    SE_matrix = robot.fkine(qpos.numpy())  # pyright: ignore[reportUnknownMemberType]

    translation = cast("NDArray[np.float64]", SE_matrix.A)[:3, 3].reshape((1, 3))
    quat = np.array(UnitQuaternion(SE_matrix).data).astype(np.float64)  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]

    return Tensor(np.concatenate([translation, quat], axis=1))


@final
class Agent(nn.Module):
    def __init__(
        self, observation_space_size: int, action_space_size: int, device: torch.device
    ):
        super().__init__()
    ...
    def get_action_from_qpos(self, qpos_: NDArray[np.float32]) -> Tensor:

        qpos = torch.Tensor(qpos_).reshape((1, 8))

        qvel = torch.zeros((1, 8))

        tcp_pose = compute_tcp_pose(self.ec_robot, qpos[:6])
...
def create_agent(checkpoint: str) -> Agent:
    ...
    model = cast(
        "OrderedDict[str, Tensor]", torch.load(checkpoint, map_location=device)
    )
